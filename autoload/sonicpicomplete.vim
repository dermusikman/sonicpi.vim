...
# Samples from server/sonicpi/lib/sonicpi/synthinfo.rb
  ...
  def get_completions(base)
    completions = []
    [@@directives, @@synths, @@fx, @@samples].each do |wordlist|
      completions += wordlist.grep(/#{base}/)
    end
    completions
  end

  # get_directives(base), get_synths... etc.
  %w(directives synths fx samples).each do |wordlist|
    m = "get_" + wordlist
    define_method(m.to_sym) do |base|
      self.send(wordlist.to_sym), base
    end
  end
